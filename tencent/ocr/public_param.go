...
// 公共请求参数
func publicParam(secretId, secretKey string, regions ...string) (js string, err error) {
	region := "ap-beijing"
	if len(regions) > 0 {
		region = ""
	}
	...
	client, _ := ocr.NewClient(common.NewCredential(secretId, secretKey), region, cpf)
